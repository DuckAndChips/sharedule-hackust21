/*
 * [Depreciated] Use ../server/index.js which uses mongoDB instead.
 * Express server that acts as a web server and also a backend.
 * Uses neDB to handle CRUD operations of schedule, to-do list and to-do list password.
 */
...
app.get("/schedule/init", (request, response) => {
...
app.post("/schedule/add", (request, response) => {
    ...
    // The request body is in schema of {title: String, color: String, start: Date, end: Date, extendedProps: {colorLabel: String}}
    scheduleDB.insert(request.body, (err, newDoc) => {
        // This updates the newly added record with the eid key with _id as value in extendedProps
        scheduleDB.update(
            { _id: newDoc._id },
    ...
    scheduleDB.update(
        { _id: editData.extendedProps.eid },
        {
            $set: {
                ...
                "extendedProps.colorLabel": editData.extendedProps.colorLabel,
            },
        },
        {}
    );
